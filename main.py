...
data['Crew_name_normalized'] = data['crew_names'].apply(lambda x: unidecode(x.lower()) if isinstance(x, str) else '')
data['Crew_job'] = data['crew_jobs'].apply(lambda x: unidecode(x.lower()) if isinstance(x, str) else '')
...
def recomendar_peliculas(titulo_pelicula: str, data, n_recomendaciones=5):
    ...
    try:
        required_columns = ['title', 'genre_name', 'company_names', 'vote_average']
